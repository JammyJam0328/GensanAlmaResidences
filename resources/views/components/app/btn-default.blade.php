...
<{{ $attributes->whereStartsWith('href')->first() ? 'a' : 'button' }} type="{{ $type }}"
    ...
    class="inline-flex items-center px-4 py-2 font-medium text-{{ $textColor }}-500 duration-100 ease-in-out focus:ring-offset-2 focus:ring-2 focus:ring-primary-700 bg-white border border-{{ $textColor }}-300 rounded-lg shadow-sm focus:shadow-md hover:bg-{{ $textColor }}-50 focus:outline-none ">
    @if ($iconRight == false)
        ...
        @if ($showLoading)
            <div wire:loading
                wire:target="{{ $wire_method }}"
                class="mr-2">
                <x-icons.spinner class="w-6 h-6 fill-primary-500 animate-spin" />
            </div>
        @endif
    @endif
    <span>
