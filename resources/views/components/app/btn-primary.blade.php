...
<{{ $attributes->whereStartsWith('href')->first() ? 'a' : 'button' }} type="{{ $type }}"
    ...
    class="inline-flex items-center justify-center px-4 py-2 font-medium text-white duration-100 ease-in-out rounded-lg shadow-sm focus:ring-offset-2 focus:ring-2 focus:ring-primary-700 bg-primary-700 focus:shadow-md hover:bg-primary-600 focus:outline-none ">
    @if ($iconRight == false)
        ...
        @if ($showLoading)
            <div wire:loading
                wire:target="{{ $wire_method }}"
                class="mr-2">
                <x-icons.spinner class="w-6 h-6 fill-white animate-spin" />
            </div>
        @endif
    @endif
    <span>
