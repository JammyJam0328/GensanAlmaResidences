<div class="space-y-3">
    <x-filament::card>
        <div class="flex items-center space-x-3">
            <x-forms.input wire:model.defer="search"
                placeholder="Search" />
            <x-forms.select wire:model.defer="searchBy">
                <option value="1">Search By (Qr Code)</option>
                <option value="2">Search By (Room)</option>
                <option value="3">Search By (Contact Number)</option>
            </x-forms.select>
            <div class="mt-1">
                <x-app.btn-primary wire:click="search"
                    label="Search"
                    showLoading>
                </x-app.btn-primary>
            </div>
    ...
    <x-filament::card>
        @if ($this->guest && $realSearch != '')
            <x-slot:header>
                <div class="flex items-center space-x-4 ">
                    <h1>Search : {{ $realSearch }}</h1> <span>|</span>
                    <h1>Search By : @if ($searchBy == '1')
                            Qr Code
                        @elseif ($searchBy == '2')
                            Room
                        @elseif ($searchBy == '3')
            ...
            <div class="space-y-3 ">
                <div class="overflow-hidden bg-white sm:rounded-lg">
                    <ul>
                        <li class="px-3 py-2 bg-white rounded-lg shadow">
                            <div class="w-full">
                                <span class="text-gray-600">Name :</span> {{ $this->guest->name }}
                            </div>
                        </li>
                        <li class="px-3 py-2 bg-white rounded-lg shadow">
                            <div class="w-full">
                                <span class="text-gray-600">Qr Code :</span> {{ $this->guest->qr_code }}
                            </div>
                        </li>
                        <li class="px-3 py-2 bg-white rounded-lg shadow">
                            <div class="w-full">
                                <span class="text-gray-600">Contact Number :</span> {{ $this->guest->contact_number }}
                            </div>
                        </li>
                    </ul>
                </div>
                <div class="p-3 border-2 border-green-500 rounded-lg ">
                    <div>
                        <h3 class="text-lg font-medium leading-6 text-gray-900">
                            Transactions
                        </h3>
                        <p class="max-w-2xl mt-1 text-sm text-gray-500">
                            List of all transactions of this guest.
                        </p>
                    </div>
                                ...
                                <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                                    <dt class="text-sm font-medium text-gray-500">
                                        ...
                                        @switch($transaction->transaction_type_id)
                                            @case(1)
                                                <span class="text-gray-800">Room #
                                                    {{ $transaction->check_in_detail->room->number }} |
                                                    {{ $transaction->check_in_detail->rate->type->name }} |
                                                    {{ Str::plural($transaction->check_in_detail->rate->staying_hour->number . ' hour', $transaction->check_in_detail->rate->staying_hour->number) }}</span>
                                            @break

                                            @case(2)
                                                <span class="text-gray-800">TV remote and Key</span>
                                            @break
                                    ...
                                    <dd class="flex mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        ...
                                        @endif
                                        <di class="flex items-center ml-4 space-x-2">
                                            ...
                                            @if ($transaction->transaction_type_id == 1 &&
                                                ...
                                                $transaction->check_in_detail->check_out_at == null)
                                                <x-app.btn-default label="Check Out"
                ...
                <div class="mt-5">
                    <h1 class="text-lg font-semibold">Balance :
                        ₱ {{ $this->guest->transactions->where('paid_at', null)->sum('payable_amount') }}</h1>
                </div>
            </div>
            <x-slot:footer>
                <div class="flex justify-end">
                    <x-app.btn-primary wire:click="totalyCheckOutGuest"
                        showLoading
        ...
        @else
            <div class="grid items-center justify-center w-full">
                <h1 class="text-2xl font-semibold animate-pulse">
                    Please search guest to view their transactions to check out.
                </h1>
                <svg data-name="Layer 1"
                    ...
                    <path
                        ...
                        transform="translate(-341.91432 -217.99034)"
                        fill="#2f2e41" />
                    <path id="b687333e-18de-4d8a-8832-7c3edcdf36cf-140"
                        data-name="Path 336"
