...
$buttonClasses = array_merge(
    ['inline-flex items-center justify-center py-2 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset filament-button', 'dark:focus:ring-offset-0' => $darkMode, 'opacity-70 cursor-not-allowed pointer-events-none' => $disabled, 'min-h-[2.25rem] px-4 ' => $size === 'md', 'min-h-[2rem] px-3 text-sm' => $size === 'sm', 'min-h-[2.75rem] px-6 text-lg' => $size === 'lg'],
    $outlined
        ? [
        ...
        : [
            ...
            'bg-primary-700 hover:bg-primary-600 focus:bg-primary-700 focus:ring-offset-primary-700' => $color === 'primary',
            'bg-success-700 hover:bg-success-600 focus:bg-success-700 focus:ring-offset-success-700' => $color === 'success',
            'bg-danger-700 hover:bg-danger-600 focus:bg-danger-700 focus:ring-offset-danger-700' => $color === 'danger',
            'bg-warning-700 hover:bg-warning-600 focus:bg-warning-700 focus:ring-offset-warning-700' => $color === 'warning',
            'bg-gray-700 hover:bg-gray-600 focus:bg-gray-700 focus:ring-offset-gray-700' => $color === 'gray',
...
$iconClasses = \Illuminate\Support\Arr::toCssClasses(['filament-button-icon', 'w-5 h-5' => $size === 'md', 'w-4 h-4' => $size === 'sm', 'w-6 h-6' => $size === 'lg', 'mr-1 -ml-2 rtl:ml-1 rtl:-mr-2' => $iconPosition === 'before' && $size === 'md' && !$labelSrOnly, 'mr-2 -ml-3 rtl:ml-2 rtl:-mr-3' => $iconPosition === 'before' && $size === 'lg' && !$labelSrOnly, 'mr-1 -ml-1.5 rtl:ml-1 rtl:-mr-1.5' => $iconPosition === 'before' && $size === 'sm' && !$labelSrOnly, 'ml-1 -mr-2 rtl:mr-1 rtl:-ml-2' => $iconPosition === 'after' && $size === 'md' && !$labelSrOnly, 'ml-2 -mr-3 rtl:mr-2 rtl:-ml-3' => $iconPosition === 'after' && $size === 'lg' && !$labelSrOnly, 'ml-1 -mr-1.5 rtl:mr-1 rtl:-ml-1.5' => $iconPosition === 'after' && $size === 'sm' && !$labelSrOnly]);
...
@if ($tag === 'button')
    <button
        ...
        @unless($disabled) x-bind:class="{ 'opacity-70 cursor-wait': isUploadingFile }" @endunless
        ...
        @elseif ($hasLoadingIndicator)
            <x-filament-support::loading-indicator wire:loading.delay
                x-cloak
        ...
        <span class="flex items-center gap-1">
            @if ($type === 'submit' && filled($form))
                <x-filament-support::loading-indicator x-show="isUploadingFile"
                    x-cloak
                    :class="$iconClasses" />
